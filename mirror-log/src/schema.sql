...
--
-- IMPORTANT: every statement is idempotent (IF NOT EXISTS) so init_db is safe to
-- run against both a fresh database and an existing one (it becomes a no-op for
-- objects that already exist). The DDL below is the single source of truth for
-- the mirror.db schema and must stay in sync with the queries in log.rs,
-- view/mod.rs, decay.rs, attention/, iteration/, catalogue.rs and distillation.rs.
--
-- Schema version is tracked via PRAGMA user_version (see db.rs). Bump it when you
-- change the shape of any table; add a migration step in db.rs rather than editing
-- existing DDL in place.
...
-- ---------------------------------------------------------------------------
-- Tags and event-to-event links (the "graph" over events)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Embeddings (semantic search) and background enrichment jobs
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Iteration tracking (multi-pass enrichment / MAL loop)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Attention / decay layer: temporal relevance, shadow state, pinning
-- ---------------------------------------------------------------------------
